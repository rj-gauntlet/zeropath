...
export default function ScanStatusCard({ scan, progress, detail }: ScanStatusCardProps) {
  return (
    <div className="bg-surface border border-border-strong rounded-lg p-5 mb-6">
      ...
      {/* Progress message */}
      <p className="text-sm text-text-muted ml-6">{progress}</p>
      {detail && <p className="text-xs text-text-faint ml-6 mt-1">{detail}</p>}

      {/* File counts */}
      {scan.files_scanned > 0 && (
        <div className="mt-3 ml-6 flex gap-4 text-xs text-text-faint">
